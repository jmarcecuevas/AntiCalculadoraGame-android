...
public class Game {
    ...
    public void advanceToNewLevel() {
        if (level == 8) {
            gameListener.onLevelsCompleted();
        } else {
            input = new Input();
            level++;
            timeInSecs -= 5;
    ...
    public void evalExpression() {
        if (!input.isEmpty()) {
            if (!input.containsAllOperators()) {
                timer.cancel();
                gameListener.gameOver("Lo siento,la expresión que ingresaste no utiliza todos los operadores que debe utilizar.", level);
            } else
    ...
    private void startTimer() {
        timer = new CountDownTimer(timeInSecs * 1000, 1000) {
            ...
            public void onFinish() {
                gameListener.gameOver("Lo siento,te has quedado sin tiempo.", level);
            }
        }.start();
    }
    ...
    public void compareInputWithGameNumber() {
        ...
        if (numberGame == calculateExpression())
            gameListener.userWon(level);
        else
            gameListener.gameOver("Lo siento, el resultado de la expresión que ingresaste no coincide con el número que debía coincidir.", level);
    }
